/*!
# Test Structure for `fs-index` Crate

This test suite is designed to validate the functionality of `fs-index` crate using different hash functions.
The crate is tested with both cryptographic and non-cryptographic hash functions
to ensure that the resource index works correctly regardless of the underlying hash function used.

## Modules

- `blake3`: This module contains tests using the `Blake3` hash function.
- `crc32`: This module contains tests using the `CRC32` hash function.
- `test_utils`: This module contains common helper macro to generate tests for different hash functions.
...
mod blake3 {
    ...
    use crate::generate_tests_for_hash;

    generate_tests_for_hash!(Blake3);
}
...
mod crc32 {
    ...
    generate_tests_for_hash!(Crc32);
}

mod test_utils {
    /// A macro to generate tests for the resource index.
    ///
    /// This macro generates tests for a given hash type. The hash type must
    /// implement the `ResourceId` trait.
    #[macro_export]
    macro_rules! generate_tests_for_hash {
        ($hash_type:ty) => {
            ...
            use crate::{
                index::IndexedResource, utils::load_or_build_index,
                ResourceIndex,
            };
            ...
            fn get_indexed_resource_from_file<P: AsRef<Path>>(
                path: P,
                parent_dir: P,
            ) -> Result<IndexedResource<$hash_type>> {
                let id = <$hash_type>::from_path(&path)?;
            ...
            fn test_store_and_load_index() {
                let temp_dir =
                    TempDir::with_prefix("ark_test_store_and_load_index")
                ...
                let index: ResourceIndex<$hash_type> =
                    ResourceIndex::build(root_path)
                        .expect("Failed to build index");
                assert_eq!(index.len(), 1);
                index.store().expect("Failed to store index");

                let loaded_index = load_or_build_index(root_path, false)
            ...
            fn test_store_and_load_index_with_collisions() {
                let temp_dir = TempDir::with_prefix(
                    "ark_test_store_and_load_index_with_collisions",
                )
                .expect("Failed to create temp dir");
                let root_path = temp_dir.path();
                ...
                fs::write(&file_path2, "file content")
                    .expect("Failed to write to file");

                let file_path3 = root_path.join("file3.txt");
                fs::write(&file_path3, "file content")
                    .expect("Failed to write to file");

                let file_path4 = root_path.join("file4.txt");
                fs::write(&file_path4, "file content")
                    .expect("Failed to write to file");
                ...
                let index: ResourceIndex<$hash_type> =
                    ResourceIndex::build(root_path)
                        .expect("Failed to build index");
                let checksum = <$hash_type>::from_path(&file_path)
            ...
            /// - Assert that the resource retrieved by path matches the
            ///   expected resource.
            /// - Assert that the resource retrieved by ID matches the expected
            ///   resource.
            #[test]
            fn test_build_index_with_file() {
                let temp_dir =
                    TempDir::with_prefix("ark_test_build_index_with_file")
                        .expect("Failed to create temp dir");
                let root_path = temp_dir.path();
                ...
                fs::write(&file_path, "file content")
                    .expect("Failed to write to file");
                let expected_resource = get_indexed_resource_from_file(
                    &file_path,
                    &root_path.to_path_buf(),
                )
                .expect("Failed to get indexed resource");

                let index = ResourceIndex::build(root_path)
                    .expect("Failed to build index");
                assert_eq!(index.len(), 1);
            ...
            fn test_build_index_with_empty_file() {
                let temp_dir = TempDir::with_prefix(
                    "ark_test_build_index_with_empty_file",
                )
                .expect("Failed to create temp dir");
                let root_path = temp_dir.path();
            ...
            fn test_build_index_with_directory() {
                let temp_dir =
                    TempDir::with_prefix("ark_test_build_index_with_directory")
                        .expect("Failed to create temp dir");
                let root_path = temp_dir.path();
            ...
            /// - Assert that the resource retrieved by path for each file
            ///   matches the expected resource.
            #[test]
            fn test_build_index_with_multiple_files() {
                let temp_dir = TempDir::with_prefix(
                    "ark_test_build_index_with_multiple_files",
                )
                .expect("Failed to create temp dir");
                let root_path = temp_dir.path();
                ...
                let expected_resource1 = get_indexed_resource_from_file(
                    &file1_path,
                    &root_path.to_path_buf(),
                )
                .expect("Failed to get indexed resource");
                let expected_resource2 = get_indexed_resource_from_file(
                    &file2_path,
                    &root_path.to_path_buf(),
                )
                .expect("Failed to get indexed resource");
            ...
            ///
            /// ## Test scenario:
            /// - Create multiple directories within the temporary directory,
            ///   each containing a file.
            /// - Build a resource index in the temporary directory.
            /// - Assert that the index contains two entries.
            /// - Assert that the resources retrieved by path for each file
            ///   match the expected resources.
            #[test]
            fn test_build_index_with_multiple_directories() {
                let temp_dir = TempDir::with_prefix(
                    "ark_test_build_index_with_multiple_directories",
                )
                .expect("Failed to create temp dir");
                let root_path = temp_dir.path();
            ...
            /// - Build a resource index in the temporary directory.
            /// - Assert that the index initially contains the expected number
            ///   of entries.
            /// - Create a new file, modify an existing file, and remove another
            ///   file.
            /// - Update the resource index.
            /// - Assert that the index contains the expected number of entries
            ///   after the update.
            /// - Assert that the entries in the index match the expected state
            ///   after the update.
            #[test]
            fn test_resource_index_update() {
                let temp_dir =
                    TempDir::with_prefix("ark_test_resource_index_update")
                        .expect("Failed to create temp dir");
                let root_path = temp_dir.path();
            ...
            /// - Assert that the index contains the expected number of entries
            ///   after the addition.
            /// - Assert index.collisions contains the expected number of
            ///   entries.
            #[test]
            fn test_add_colliding_files() {
                let temp_dir =
                    TempDir::with_prefix("ark_test_add_colliding_files")
                        .expect("Failed to create temp dir");
                let root_path = temp_dir.path();
                ...
                let mut index: ResourceIndex<$hash_type> =
                    ResourceIndex::build(root_path)
                        .expect("Failed to build index");
                index.store().expect("Failed to store index");
                assert_eq!(index.len(), 1);
            ...
            /// - Assert that the index initially contains the expected number
            ///   of entries.
            /// - Create 2 new files with the same checksum as the existing
            ///   file.
            /// - Update the index.
            /// - Assert that the index contains the expected number of entries
            ///   after the update.
